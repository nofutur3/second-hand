...
- **`src/frontend`** — Nuxt 3 / Vue 3 app with two pages: `pages/index.vue` (list of saved searches) and `pages/search/[id].vue` (products for a search), fetching from `NUXT_PUBLIC_API_BASE` (defaults to the Docker-network API host). `pages/*-old.vue` files are superseded and unused.
